...
!INC ArchiMate.Style Layout Connectors
!INC Logging.LogManager

sub main
	dim logger
	set logger = LogManager.getLogger("ArchiMate.Do Style Layout Connectors")
	...
	if not diagram is nothing then
		...
		for each diagramObject in diagram.SelectedObjects		
			...
			applyStyleLayoutConnectors myArchiMateElement
		next
	end if
	logger.INFO "Done"
end sub
